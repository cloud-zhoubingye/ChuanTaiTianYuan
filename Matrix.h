...
public:
    ...
    */
    ...
    //Pay Attention!
    //This function may have high costs!
    //Be cautious to use it!
    {
        if (i==j) return;
        //apply for a new matrix
        int row_cnt=1;
        int line_cnt=(*this).dim1();
        Matrix<T,1> temp(row_cnt,line_cnt);
        //copy
        temp=*this;
        //swap
...
public:
    ...
    */
    ...
    //Be cautious to use it!
    {
        ...
        //apply for a new matrix
        int row_cnt=(*this).sz/(*this).dim1();
        int line_cnt=(*this).dim1();
        Matrix<T,2> temp(row_cnt,line_cnt);
        ...
        //swap
        for (int row_index = 0; row_index < row_cnt; ++row_index) {
            temp(row_index,i)=(*this)(row_index,j);
        }
        for (int row_index = 0; row_index < row_cnt; ++row_index) {
            temp(row_index,j)=(*this)(row_index,i);
        }

        //set new matrix
        *this=temp;
    }
    friend Matrix<T,2> operator*(const Matrix<T,2>&m1, const Matrix<T,2>&m2)
    {
        //Find the number of rows in the matrix
        int row_cnt1=m1.sz/m1.dim1();
        int col_cnt1=m1.dim1();
        int col_cnt2=m2.dim1();

        //Check whether multiplication is allowed
        if (row_cnt1!=col_cnt2)
            error("Two matrices cannot do multiplications!");
        //Create return matrix
        Matrix<T,2> result(row_cnt1,col_cnt2);

        //Calculate the product
        for (int row = 0; row < row_cnt1; ++row) {
            for (int col = 0; col < col_cnt2; ++col) {
                //Calculate the inner product of vectors
                T product=0;
                for (int i = 0; i < col_cnt1; ++i) {
    ...
    friend Matrix<T,2> operator+(const Matrix<T,2>&m1, const Matrix<T,2>&m2)
    {
        ...
        int row_cnt1=m1.sz/m1.dim1();
        int col_cnt1=m1.dim1();
        int row_cnt2=m2.sz/m2.dim1();
        int col_cnt2=m2.dim1();

        //Check whether addition is allowed
        if (row_cnt1!=row_cnt2||col_cnt1!=col_cnt2)
            error("Two matrices cannot do multiplications!");

        //Create return matrix
        Matrix<T,2> result(row_cnt1,col_cnt1);
